...
xcodebuild -configuration Release -target libochamcrest -sdk iphoneos > /dev/null
OUT=$?
if [ "${OUT}" -ne "0" ]; then
	echo Device build failed
	exit ${OUT}
fi
xcodebuild -configuration Release -target libochamcrest -sdk iphonesimulator > /dev/null
OUT=$?
if [ "${OUT}" -ne "0" ]; then
